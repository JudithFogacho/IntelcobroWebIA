...
/**
 * Configuración de las secciones de la rueda con sus propiedades
 */
export interface WheelSectionConfig {
  section: WheelSection;
  label: string;
  discountPercentage: number;
  probability: number; // Probabilidad en porcentaje (0-100)
  color: string;
  textColor: string;
}
 ...
 */
export const WHEEL_SECTIONS_CONFIG: WheelSectionConfig[] = [
  {
 ...
 */
export function getSectionConfig(section: WheelSection): WheelSectionConfig | undefined {
  return WHEEL_SECTIONS_CONFIG.find(config => config.section === section);
}

/**
 * Función helper para validar si un string es una WheelSection válida
 */
export function isValidWheelSection(section: string): section is WheelSection {
  return Object.values(WheelSection).includes(section as WheelSection);
}

/**
 * Función helper para obtener todas las secciones con descuento (excluyendo TRY_AGAIN)
 */
export function getDiscountSections(): WheelSectionConfig[] {
  return WHEEL_SECTIONS_CONFIG.filter(config => config.discountPercentage > 0);
}